      ...
      <b-card class="main-card mb-4">
        ...
        >
          ...
          <template v-slot:cell(remarks)="row">
            <div>
              <b-form-input v-model="row.item.remarks" class="d-inline" style="width:83%" placeholder="Remarks" />
              <v-icon @click="saveRemarks(row.item.vehicleNo,row.item.remarkId,row.item.remarks)">save</v-icon>
            </div>
          </template>
...
export default {
  ...
  data() {
    return {
      ...
      fields: [
        ...
        { key: "remarks", label: "Remarks", sortable: false, thClass: 'text-center', tdClass: 'text-center', }
      ],
      tableSettings: {
        currentPage: 1,
        perPage: 100,
  ...
  methods: {
    ...
    async saveRemarks(vehicleNo,rmId,remarks){
      let uid = wialon.core.Session.getInstance().getCurrUser()._id
      if(rmId != null){
        db.collection("remarks").doc(rmId).update({
          remarks: remarks
        })
      }else{
        db.collection("remarks").add({
          ...
          let unit = this.units.find(u=>u.vehicleNo == vehicleNo)
          if(unit){
            unit.remarkId = ref
          }
        })
    ...
    async getRemarks(){
      let uid = wialon.core.Session.getInstance().getCurrUser()._id
      return await db.collection("remarks").where("userId","==",uid).get().then(snapshot => {
        this.remarks = snapshot.docs.map(doc=>Object.assign({id:doc.id},doc.data()))
        return this.remarks
      })
    },
    checkUser(){
    ...
    async getUnits(isFirstTime) {
      ...
      // console.log("called")
      let _this = this;
      if(isFirstTime) _this.getRemarks()
                ...
                try {
                  ...
                  if(isFirstTime){
                    ...
                    try{
                      ...
                      obj.startTime = un.startTime
                      obj.todayMileage = un.todayMileage
                      obj.idlling = un.idlling
                    }catch(locEx){
                      obj.locationAddress = '-'
                    }
                  }
